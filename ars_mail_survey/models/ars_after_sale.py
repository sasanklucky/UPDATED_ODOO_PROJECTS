...
class AccountInvoice_inherit(models.Model):
    ...
    def _determine_user_to_assign(self, type):
        ...
        member_ids = sorted(cre_team_id.team_member_ids.ids) if cre_team_id.team_member_ids else sorted(
            cre_team_id.team_member_ids.ids)
        assigned_user_id = False
        if member_ids:
            if cre_team_id.assign_method == 'randomly':
                last_assigned_user = self.env['mail.activity'].search([('cre_id', '=', cre_team_id.id)],
    ...
    def action_invoice_open(self):
        ...
        if not sale_order_search:
            # and not self.partner_id.opt_out
