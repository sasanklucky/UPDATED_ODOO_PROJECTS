        ...
        try:
            ...
            if child == 'is_child_company' and check_quotation_sync == 'yes':
                ...
                with contextlib.closing(db.cursor()) as cr:
                    ...
                    exist_in_parent = env['crm.lead'].sudo().search(
                        [('child_id_ref', '=', str(child_id)), ('child_db', '=', child_database)])
            ...
            if child == 'is_child_company' and check_pipeline_sync == 'yes':
                ...
                with contextlib.closing(db.cursor()) as cr:
                    ...
                    if crm_rec:
                        ...
                        pipelines = self.env['crm.lead'].sudo().search(
                            [('sync_pipeline', '=', False), '|', ('active', '=', True), ('active', '=', False)],
                            order='id asc')
                    # print("===",pipelines)
                    for rec in pipelines:
                        ...
                        exist_in_parent = lead.search(
                            [('child_id_ref', '=', str(rec.id)), ('child_db', '=', child_database), '|',
                             ('active', '=', True), ('active', '=', False)], limit=1, order='id desc')
                        ...
                        company = env['res.company'].sudo().search([('dealer_code', '=', rec.company_id.dealer_code)],
                        ...
                        if rec.partner_id:
                            customer = env['res.partner'].sudo().search(
                                [('mobile', '=', rec.partner_id.mobile), ('company_id', '=', company.id)],
                                order='id desc', limit=1)
                        # if new_partner_id:
                        #     customer = new_partner_id

                        title = env['res.partner.title'].sudo().search([('name', '=', rec.title.name)], order='id desc',
                                                                       limit=1)
                        ...
                        country = env['res.country'].sudo().search([('name', '=', rec.country_id.name)],
                                                                   order='id desc', limit=1)
                        ...
                        state = env['res.country.state'].sudo().search([('name', '=', rec.state_id.name)], limit=1,
                                                                       order='id desc')
                        ...
                        tag = env['crm.lead.tag'].sudo().search([('name', 'in', rec.tag_ids.mapped('name'))],
                                                                order='id desc', limit=1)
                        ...
                        campaign = env['utm.campaign'].sudo().search([('name', '=', rec.campaign_id.name)],
                                                                     order='id desc', limit=1)
                        ...
                        medium = env['utm.medium'].sudo().search([('name', '=', rec.medium_id.name)], order='id desc',
                                                                 limit=1)
                        ...
                        source = env['utm.source'].sudo().search([('name', '=', rec.source_id.name)], order='id desc',
                                                                 limit=1)
                        ...
                        stage_id = env['crm.stage'].sudo().search([('name', '=', rec.stage_id.name)], order='id desc',
                                                                  limit=1)
                        ...
                        for line_data in rec.vehicle_line:
                            if line_data:
                                template_data = env['product.template'].sudo().search(
                                    [('name', '=', line_data.product_template_id.name),
                                     ('default_code', '=', line_data.product_template_id.default_code)],
                                    order='id desc', limit=1)
                                ...
                                catalog_data = env['product.catalog'].sudo().search(
                                    [('name', '=', line_data.product_catalog_id.name)], order='id desc', limit=1)
