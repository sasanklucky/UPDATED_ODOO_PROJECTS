...
class sales_report_format(models.Model):
    ...
    def get_cgst_amount(self):
        for record in self:
            ...
            tax_records = self.env['account.tax'].sudo().search([('id','in',list_tax),('tax_group_id','=',tax_group.id)])
            if tax_records: 
                percent = 0
    ...
    def init(self):
        ...
        print(self.env.cr.execute("""select row_number() over() as id,a.id as invoice_id,a.company_id,
