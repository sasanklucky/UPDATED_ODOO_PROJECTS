...
class ARS_sale_order_line(models.Model):
    ...
    apr_action = fields.Selection([('approved','Approved'),('reject','Reject'),('hold','Hold'),('re_submission','Re Submission')], string='Action')
    ...
    def CustomerSplit_Change(self):
        ...
        if self.customer_split and self.product_id:
           res = self.customer_split.property_product_pricelist.with_context().get_product_price_rule(self.product_id, 1.0, self.customer_split)
    ...
    def write(self, values):
        ...
        if 'apr_action' in values and not self.customer_split and values.get('apr_action') == 'approved':
            ...
            if params.get('model') == 'ars.sale.warranty':
               warranty = self.env['ars.sale.warranty'].search_read([('id', '=', params.get('id'))], fields=['partner_id'])
               if warranty:
