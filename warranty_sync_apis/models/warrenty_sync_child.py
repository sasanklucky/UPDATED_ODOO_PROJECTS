...
class ArsSaleWarrentySync(models.Model):
    ...
    def _compute_any_line_rejected(self):
        for record in self:
            record.any_line_rejected = any(line.apr_action == 'reject' for line in record.order_lines)
    ...
    #                 record.hide_sync = True
            ...
            #     record.sync_warranty = False  # Ensure sync_warranty is set to False
            #
            #     # Update the apr_action field in order_lines
            #     for line in record.order_lines:
            #         line.apr_action = False
            #     record.order_lines.write({'apr_action': False})
    ...
    def sync_warranty_to_parent(self, current_warrenty_record):
        ...
        print(current_warrenty_record, 'current_warrenty_record')
        if child == 'is_child_company' and check_warrenty_sync == 'yes':
            ...
            with contextlib.closing(db.cursor()) as cr:
                ...
                print(env, 'env_created_successfully...')
                ...
                if current_warrenty_record:
                    ...
                    warrenty_records = self.env['ars.sale.warranty'].sudo().search(
                        [('sync_warranty', '=', False), ('state', '=', 'draft')], order='id asc')
                ...
                for rec in warrenty_records:
                    print("rec=====", rec)
                    stop_sync = False
                    print("2222", stop_sync)
                    sync_log_dict = {
                        'warrenty_record': rec.id,
                        'warrenty_sequence': rec.name,
                        'status': 200,
                        # 'sync_message': 'Success.Warranty synced to Parent.',
                    }
                    print("33333", sync_log_dict)
                    # parent record env
                    sale_quotation = env['ars.sale.warranty'].sudo()
                    print("444444", sale_quotation)
                    # print(sale_quotation,'sale_quotationsale_quotationsale_quotation')
                    exist_in_parent = sale_quotation.search(
                        [('child_warrenty_id_ref', '=', str(rec.id)), ('child_db', '=', child_database)], limit=1,
                        order='id desc')

                    print("exist_in_parentexist_in_parentexist_in_parent", exist_in_parent,self.child_warrenty_id_ref )
                    if not exist_in_parent:
                        ...
                        if data and not stop_sync:
                            ...
                            sync_log_dict['payload'] = {'warrenty_sequence': rec.name, 'db': child_database, 'values': data}
