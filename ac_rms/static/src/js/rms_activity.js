odoo.define('ac_rms.rms_activity', function (require) {
"use strict";
...
var KanbanActivity = AbstractActivityField.extend({
    ...
    events: {
        ...
        'click .o_start_clk': '_onStartClick',
        'click .o_finish_clk': '_onFinishClick',
    },
    ...
    _onStartClick1: function (id) {
     ...
     _onStartClick: function (event) {
        event.stopPropagation();
        var activity_id = $(event.currentTarget).data('activity-id');
        this._onStartClick1(activity_id).then(function (val) {
                    if(val['status']){
                        location.reload();
//                        $(".pull-right").empty();
//                        $(".pull-right").append('<span class="o_finish_clk btn btn-primary" style="padding: 4px 6px;font-size: 11px;" string="Finish" name="finish_activity" t-att-data-activity-id="'+activity_id+'">Finish</span>')
                    }
                });
    },

    _onFinishClick1: function (id) {
        //var activity_id = $(event.currentTarget).data('activity-id') || false;
        //console.log(id);
        /*if(id){*/
            return this._rpc({
     ...
     _onFinishClick: function (event) {
        event.stopPropagation();
        var activity_id = $(event.currentTarget).data('activity-id');

        this._onFinishClick1(activity_id).then(this._reload.bind(this));
    },
