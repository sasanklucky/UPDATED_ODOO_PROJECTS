...
//				var chr = todayDate.getHours() - 1;
//				if (chr == hour_ && min_ < todayDate.getMinutes()){
//				    $o.addClass('active');
//				}
				...
				if (date.getMinutes() > 5){
				    //alert('min');
				    var quotient = Math.floor(date.getMinutes()/5);
	        	    var remainder = date.getMinutes() % 5;
	        	    var cus_min = quotient * 5;
	        	    //alert(cus_min);
				}else{
				    var cus_min = 5;
				}
                var nxt_cus_min = cus_min+5;
                //alert(nxt_cus_min);
				if (hour_ == date.getHours()-1 && (cus_min <= min_)) { /* selected time */
				    //alert(cus_min);
				    //alert(min_);
                    if(nxt_cus_min > min_){
                        $o.addClass('active');
...
//
		...
		if( 0 <= opt.minuteInterval && opt.minuteInterval <= 5 ){
		    alert(opt.minuteInterval);
			$picker.data("minuteInterval", opt.minuteInterval);
		} else {
			$picker.data("minuteInterval", 5);
		}
