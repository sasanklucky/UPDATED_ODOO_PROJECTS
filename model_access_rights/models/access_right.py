...
class AccessRight(models.Model):
    ...
    def hide_buttons(self, **kwargs):
        ...
        """Query access rights to determine which buttons to hide"""
        access_rights = self.sudo().search_read([], ['model_id', 'is_delete', 'is_export', 'is_create_or_update',
                                                     'is_archive', 'groups_id'])

        for rec in access_rights:
            model_id = self.env['ir.model'].sudo().browse(rec['model_id'][0]).model
            if str(model_id) == model_name:
                user_groups = self.env['res.users'].browse(user_id).groups_id.ids
                if rec['groups_id'][0] not in user_groups:
                    return {
